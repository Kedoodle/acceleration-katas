...
namespace MontyHall
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine(
                    $"Simulating {gamesToSimulate} games with the contestant always deciding to: {contestantDecision}");

                var gameSimulator = new GameSimulator(randomGenerator);
                gameSimulator.Simulate(contestantDecision, gamesToSimulate);

                Console.WriteLine(
                    $"The contestant won {gameSimulator.ContestantWins} out of {gameSimulator.GamesSimulated} games ({gameSimulator.GetContestantWinPercentage():P})");
            }
        }
    }
}